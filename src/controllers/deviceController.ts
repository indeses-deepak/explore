...
const createDevice = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    client.on("qr", async (qr: string) => {
      ...
      console.log("serve qr", JSON.stringify(qr));
    ...
    client.on("authenticated", async () => {
      // clientStatus[deviceId] = "AUTHENTICATED";
      logger.info(`[${deviceId}] Client authenticated.`);
...
const callWebStatus = async (
  ...
  if (status === "DISCONNECTED" || status === "READY") {
    // const userInfo = await client.getContactById(client.info.wid._serialized);
    logger.error(`Device ${status}: ${deviceId}`, {
      deviceId,
...
const checkDeviceStatusOld = async (
  req: Request,
  res: Response
): Promise<void> => {
  const { deviceId } = req.body;
...
const checkDeviceStatus = async (
  ...
  try {
    ...
    if (!client) {
      res.status(200).json({
        status: 322,
        msg: `Device with ID '${deviceId}' not found.`,
      });
      return;
    }

    const isReady = client.info?.wid ? true : false; // or client?.isConnected() or custom `client.ready` flag
    console.log(isReady, "isReady");
    console.log(client, "client");
    console.log(JSON.stringify(client.info), "client");
    logger.info(
      `No client found for device ${deviceId}` + JSON.stringify(client.info)
    );

    let device_status = status[deviceId];
    if (!isReady) {
      if (device_status === "ready") {
        device_status = "disconnected";
      }
      //device_status = 'ready'; //remove this line in production
      res.status(200).json({
        status: 200,
        deviceId,
        device_status,
        msg: `Device '${deviceId}' is found but not ready (not authenticated or connected).`,
      });
      return;
    }
    ...
    res.status(200).json({
      ...
      deviceId,
      device_status,
      msg: "Device is ready and connected.",
    });
  } catch (error) {
    const errorMessage =
      error instanceof Error ? error.message : "Unknown error occurred";
    ...
    res.status(500).json({
      status: 500,
      msg: "Failed to check device status.",
    });
  }
};
...
const executeWhatsAppMethod = async (
  ...
  if (!deviceId || !methodName) {
    res
      .status(400)
      .json({ status: 400, msg: "Missing deviceId or methodName" });
  ...
  if (!actualMethod) {
    res
      .status(403)
      .json({ status: 403, msg: `Method ${methodName} not allowed.` });
    return;
  }

  try {
    console.log(
      `Attempting to call method: ${actualMethod} with argsName:`,
      argsName
    );
...
const logoutCommon = async (deviceId: string) => {
  let success = true;

  try {
    const client = clients[deviceId];
    if (!client) {
      logger.info(`No client found for device ${deviceId}`);
      return true;
    }
    ...
    try {
      if (client.pupBrowser?.isConnected()) {
        ...
        logger.info(`Puppeteer browser already closed for device ${deviceId}`);
      }
    } catch (closeErr: any) {
      logger.info(
        `Failed to close Puppeteer browser for ${deviceId}: ${closeErr?.message}`
      );
    }

    await client.logout(); // Triggers SafeLocalAuth.logout
    await client.destroy();
    logger.info(`Successfully disconnected and cleaned up device ${deviceId}`);
  } catch (err) {
    success = false;
    ...
    if (
      ...
      logger.info(
        `Puppeteer session already closed for ${deviceId}: ${error.message}`
      );
    } else {
      logger.info(
        `Error during disconnect cleanup for ${deviceId}: ${error.message}`
      );
    }
  } finally {
    logger.info(
      `logoutCommon completed for device ${deviceId} with success: ${success}`
    );
  }
